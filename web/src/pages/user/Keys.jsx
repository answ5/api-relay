    ...
    <div>
      ...
      {newKey && (
        <div className="user-newkey-box">
          <h3>✅ API Key 创建成功！</h3>
          <p className="user-hint">请立即保存此 Key，关闭后将无法再次查看。</p>
      ...
      <div className="user-create-key-box">
        <div className="user-create-key-row">
          <div className="form-group" style={{ flex: 1, marginBottom: 0 }}>
            <label>备注名称</label>
            <input value={name} onChange={(e) => setName(e.target.value)}
              placeholder="例如：我的测试 Key" className="user-input" />
          </div>
      ...
      {loading ? <div className="user-empty">加载中...</div> : (
        <div className="user-card">
          <table className="user-table">
            ...
            <tbody>
              {tokens.length === 0 ? (
                <tr><td colSpan={8} className="user-empty-row">暂无 API Key</td></tr>
              ) : tokens.map((t) => (
                <tr key={t.id}>
                  <td>{t.name || '-'}</td>
                  <td><code className="user-code">sk-{t.key_prefix}...</code></td>
                  <td><span className={`user-badge ${t.status === 1 ? 'user-badge-green' : 'user-badge-red'}`}>
                    {t.status === 1 ? '启用' : '禁用'}
                  </span></td>
                  <td>{t.group_name}</td>
                  <td>¥{parseFloat(t.balance_limit || 0).toFixed(2)}</td>
                  <td className="user-muted">{t.last_used_at || '-'}</td>
                  <td className="user-muted">{t.created_at || '-'}</td>
                  <td>
